...
export default {
  ...
  data () {
    return {
      ...
      tableHeadData: [
        { label: '', prop: 'ac_date' },
        { label: '收入金额', prop: 'inAmt' },
        { label: '支出金额', prop: 'outAmt' },
        { label: '轧差金额', prop: 'netValue' }
      ],
      pagesize: 20
    }
  },
  methods: {
    ...
    getDataLine (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        dateRange.push(data[i].ac_date)
        profit.push(data[i].netValue)
        income.push(data[i].inAmt)
        expenses.push(data[i].outAmt)
      }
      return {
        dateRange: dateRange,
        profit: profit,
    ...
    getDataBar (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        respData[i + 1] = []
        respData[i + 1].push(data[i].ac_date)
        respData[i + 1].push(data[i].inAmt)
        respData[i + 1].push(data[i].outAmt)
    ...
    // 切换日期方式
    cycleChange ({ cycle }) {
      this.tableHeadData[0].label = this.handleCycleTitle({ cycle })
      switch (cycle) {
        case '01':
    ...
    submitHandler (formModel) {
      ...
      httpPost('/eweb-cash.CashInoutCollectQry.do', params).then(res => {
  ...
  mounted () {
    this.tableHeadData[0].label = this.handleCycleTitle(this.formModel)
