...
export default {
  ...
  methods: {
    ...
    initData (obj) {
      ...
      this.initOperating({ key: this.formModel.dGatherFlag })
    },
    initOperating ({ key }) {
      this.$nextTick(() => {
        switch (key) {
    ...
    inputValueLimit1 (obj) {
      obj.dTerTianStart = obj.dTerTianStart !== '' && Number(obj.dTerTianStart) <= 1 ? 1 : Number(obj.dTerTianStart) >= 30 ? 30 : obj.dTerTianStart
    },
    inputValueLimit2 (obj) {
      obj.dTerTianDays = obj.dTerTianDays !== '' && Number(obj.dTerTianDays) <= 1 ? 1 : Number(obj.dTerTianDays) >= 30 ? 30 : obj.dTerTianDays
    }
  },
  created () {
