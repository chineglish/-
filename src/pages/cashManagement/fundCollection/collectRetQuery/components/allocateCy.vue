...
export default {
  ...
  created () {
    ...
    if (this.formModel.dGatherFlag === '03') {
      this.formModel.dTerTianStart = this.data.dTerTianStart
      this.formModel.dTerTianDays = this.data.dTerTianDays
    } else {
      this.formModel.dTerTianStart = ''
      this.formModel.dTerTianDays = ''
    }

    // this.formModel.dMonthEndFlag = this.data.dMonthEndFlag
    let week = this.data.dWeeksCode.split('')
    week.forEach((item, index) => {
      item === '1' && this.formModel.dWeeksCode.push(this.weeks[index])
    })
    console.log(this.formModel.dWeeksCode)
