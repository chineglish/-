...
export default {
    ...
    submitTemplate () {
      if (this.templateName.length === 0) {
        ...
        this.checkedList.forEach(item => {
          if (item.value) { templateList[item.key] = item.value } else {
            this.$msg('项目内容不可上传为空')
            errorFlag = true
          }
        })
        if (!errorFlag) {
          httpPost('/eweb-common.GenToken.do').then(token => {
            httpPost('/eweb-transfer.PaySalaryTemplateModify.do', {
  ...
  created () {
    this.$route.params.templateContent.split('|').forEach(item => {
      const key = item.split('_')[0]
      const value = item.split('_')[1]
      if (key) {
        this.checkList.requiredList.forEach(required => {
          if (key === required.key) {
            required.value = value
            this.checkedList.push(required)
          }
        })
        this.checkList.sendList.forEach(send => {
          if (key === send.key) {
            send.value = value
            this.checkedList.push(send)
          }
        })
        this.checkList.deductList.forEach(deduct => {
          if (key === deduct.key) {
            deduct.value = value
            this.checkedList.push(deduct)
          }
        })
      }
    })
