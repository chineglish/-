...
export default {
    ...
    return {
      ...
      formConfigJson: {
        rules: {
          ...
          stdGuarTyp: [{ required: true, message: '保证类型', trigger: 'submit' }]
        },
        formItems: [
          {
            formWidth: '100%',
  ...
  methods: {
    ...
    submit (data) {
      ...
      if (!this.isMore) {
        params = {
          stdQryCont: data.stdGuarTyp, // 查询内容编号
      ...
      httpPost('eweb-edraft.CustomerQry.do', params).then(res => {
        if (res.list.length > 0) {
          // params增加acName 录入页,确定页用来展示使用
          params.acName = this.accNoList[data.stdCustAcc].acName
          this.$router.push({
