...
import { bill_Type, billStatus } from '@/assets/js/entity'
import { httpPost } from '@/api/sys/http'
export default {
  name: 'EndorsementTransferRevokeInquire',
  data () {
    return {
      breadData: ['电子商业汇票 ', '背书转让', '背书撤回'],
      operateData: {
        btnData: [
      ...
      tableHeadData: [
        ...
        { label: '状态', prop: 'stdBilsTat', formatter: (row, column, cellValue, index) => util.handleEnums(billStatus, cellValue) },
        { label: '审核状态', prop: 'authState' }
      ],
      tableData: [
      ],
      actionData: [
        { btnText: '背书撤回', class: 'm-submit-btn', type: 'info', eventName: 'enterSolo' }
      ]
    }
  },
  methods: {
