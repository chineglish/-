  ...
  data () {
    return {
      ...
      formConfigJson: {
        stepsActive: 0,
        rules: {
          payAgreeMoney: [{ required: true, message: '同意清偿金额 ', trigger: 'change' },
            ...
            { validator: (rule, value, callback) => {
              if (Number(value) > Number(this.formModel.stdPmMoney)) {
                callback(new Error('同意清偿金额应小于票面金额'))
              } else {
                callback()
              }
            },
  ...
  methods: {
    submit (data) {
      let param = {
        ...
        stdAgpyDat: this.formModel.agreePayDate, // 同意清偿日期
        stdApayAmt: data.payAgreeMoney, // 同意清偿金额,
        stdPpayAmt: data.stdRcrsAmt // 追索金额
      }
      httpPost('eweb-edraft.AgreePayOffConfirm.do', param).then(res => {
        this.$router.push({
