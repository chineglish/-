    ...
    return {
      ...
      formConfigJson: {
        rules: {
          ...
          stdBussTyp: [{ required: true, message: '追索类型', trigger: 'change' }]
        },
        formItems: [
          {
            formWidth: '100%',
  ...
  methods: {
    moreConditions () {
      if (!this.isMore) {
        ...
        this.formConfigJson.formItems[0].group[9].show = false
    ...
    submit (data) {
      let params = {}
      if (this.isMore) {
        if (data.stdBussTyp === 'RT00') {
          params = {
            stdQryCont: '19', // 查询内容编号(拒付追索)
    ...
    reset (res) {
      ...
      this.formModel.stdBussTyp = 'RT00'
