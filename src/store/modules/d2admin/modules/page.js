...
export default {
  ...
  //     // }).map(e => e.name)
  //     return state.opened.filter(item => {
  //       return item.meta.hasOwnProperty('notCache') && item.meta.notCache
  //     }).map(e => e.name)
  //   }
  // },
  mutations: {
      ...
      if (data !== undefined) {
        ...
        if (str === 'Object') {
          Object.keys(data).forEach(key => {
            state.errList.push(data[key][0])
          })
        } else {
          state.errList.push({ message: data })
        }
        if (errorBox) {
          window.scrollTo(util.getElementLeft(errorBox), util.getElementTop(errorBox))
          errorBox.innerHTML = `<li class="el-icon-warning-outline" /><li class="error-title">错误信息：</li><li class="warn-message">${state.errList[0].message}</li>`
        } else {
          MessageBox(state.errList[0].message, '错误信息', {
    ...
    // 移除保存的缓存页面列表
    removeKeepAliveList (state) {
      state.keepAliveList = []
    },
    // 更新需要缓存的页面列表
    updateKeepAliveList (state, name) {
